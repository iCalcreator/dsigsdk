...
use Kigkonsult\DsigSdk\DsigLoader\ManifestType;
 ...
 * Class TestManifest
 */
class TestManifest extends BaseTest
{

    public static function setUpBeforeClass()
    {
        if( defined( 'LOG' ) && ( false !== LOG )) {
            $fileName = 'TestManifest.log';
    ...
    /**
     * Create full manifest-instance loaded with Faker-data, write xml1 and parse again, write xml2 and compare
     *
     * @test
     */
    public function manifestTest3() {

        echo PHP_EOL . ' START  ' . __FUNCTION__ . PHP_EOL;
        $startTime = microtime( true );               // ---- load
        $manifest1 = ManifestType::loadFromFaker();
        echo sprintf( '%s load time    : %01.6f', __FUNCTION__, ( microtime( true ) - $startTime )) . PHP_EOL;

        $manifest1->setXMLattribute( 'xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance' );
        $manifest1->setXMLattribute( 'xmlns:xsd', 'http://www.w3.org/2001/XMLSchema' );
        $manifest1->setXMLattribute( 'xsi:schemaLocation', 'http://www.w3.org/2000/09/xmldsig# http://www.w3.org/2000/09/xmldsig#' );
        $manifest1->setXMLattribute( 'xmlns', 'http://www.w3.org/2000/09/xmldsig#' );

        $startTime = microtime( true );               // ---- write XML
        $xml1      = DsigWriter::factory()->write( $manifest1 );
